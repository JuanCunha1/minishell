/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   states.c                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: jmarques <jmarques@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/02/17 10:54:18 by jmarques          #+#    #+#             */
/*   Updated: 2026/02/17 10:54:22 by jmarques         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <lexer.h>

static void	emit_operator(t_lexer *lx, int len)
{
	t_token	*tok;

	emit_word(lx);
	tok = create_token(ft_substr(lx->input, lx->i, len),
			get_operator_type(lx->input + lx->i));
	adding_token(&lx->tokens, tok);
	lx->i += len - 1;
}

static void	normal_state(t_lexer *lx, const char *input, char c)
{
	int	len;

	len = operator_len(input + lx->i);
	if (c == '\\' && input[lx->i + 1])
	{
		lx->i++;
		buf_add(lx, input[lx->i]);
	}
	else if (len)
		emit_operator(lx, len);
	else if (isspace((unsigned char)c))
		emit_word(lx);
	else if (c == '$')
		buf_add_var(lx, input);
	else
		buf_add(lx, c);
}

static void	single_quote(t_lexer *lx, char c)
{
	if (c == '\'')
		lx->state = LX_NORMAL;
	else
		buf_add(lx, c);
}

static void	double_quote(t_lexer *lx, char c, const char *input)
{
	if (c == '"')
		lx->state = LX_NORMAL;
	else if (c == '\\' && input[lx->i + 1] != '\0'
		&& ft_strchr("\"\\$`", input[lx->i + 1]))
	{
		lx->i++;
		buf_add(lx, input[lx->i]);
	}
	else if (c == '$')
		buf_add_var(lx, input);
	else
		buf_add(lx, c);
}

void	states_loop(t_lexer *lx, const char *input)
{
	char	c;

	c = input[lx->i];
	if (lx->state == LX_NORMAL)
	{
		if (c == '\'')
		{
			lx->state = LX_IN_SQUOTE;
			lx->had_quotes = 1;
		}
		else if (c == '"')
		{
			lx->state = LX_IN_DQUOTE;
			lx->had_quotes = 1;
		}
		else
			normal_state(lx, input, c);
	}
	else if (lx->state == LX_IN_SQUOTE)
		single_quote(lx, c);
	else if (lx->state == LX_IN_DQUOTE)
		double_quote(lx, c, input);
	lx->i++;
}
